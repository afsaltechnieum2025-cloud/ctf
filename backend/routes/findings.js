...
//  Content fields: author or admin/manager only.
//  Retest-only: project assignees, author, or admin/manager.
// ─────────────────────────────────────────────────────────────
router.patch('/:id', requireAuth, async (req, res) => {
  ...
  const CONTENT_FIELDS = new Set([
    'title', 'description', 'severity', 'cvss_score', 'status',
    'steps_to_reproduce', 'impact', 'remediation', 'affected_component', 'cwe_id',
    'finding_type', 'file_path', 'line_number', 'tool_name',
    'asset_type', 'port', 'protocol', 'llm_category', 'prompt_example',
  ]);
  const RETEST_FIELDS = new Set(['retest_status', 'retest_date', 'retest_notes', 'retested_by']);

  const keys = Object.keys(updates);
  const touchesContent = keys.some((k) => CONTENT_FIELDS.has(k));
  const touchesRetest = keys.some((k) => RETEST_FIELDS.has(k));
  ...
  try {
    const [findingRows] = await db.query(
      'SELECT created_by, project_id FROM findings WHERE id = ?',
      [req.params.id]
    );
    if (findingRows.length === 0) return res.status(404).json({ message: 'Finding not found' });

    const isOwner = String(findingRows[0].created_by) === String(req.user.id);
    const isAdminOrManager = ['admin', 'manager'].includes(req.user.role);

    if (touchesContent && !isOwner && !isAdminOrManager) {
      return res.status(403).json({
        message: 'Forbidden: Only the author or an administrator can edit this finding',
      });
    }

    if (touchesRetest && !isOwner && !isAdminOrManager) {
      const [assignRows] = await db.query(
        'SELECT 1 FROM project_assignments WHERE project_id = ? AND user_id = ? LIMIT 1',
        [findingRows[0].project_id, req.user.id]
      );
      if (assignRows.length === 0) {
        return res.status(403).json({
          message: 'Forbidden: You cannot update retest status for this finding',
        });
      }
    }
...
//  Authors delete their own; admins/managers can delete any
// ─────────────────────────────────────────────────────────────
...
router.post('/:id/pocs', requireAuth, upload.single('file'), async (req, res) => {
  ...
  try {
    const [fr] = await db.query('SELECT created_by FROM findings WHERE id = ?', [req.params.id]);
    if (fr.length === 0) return res.status(404).json({ message: 'Finding not found' });
    const isOwner = String(fr[0].created_by) === String(req.user.id);
    const isElevated = ['admin', 'manager'].includes(req.user.role);
    if (!isOwner && !isElevated) {
      return res.status(403).json({ message: 'Forbidden: Only the finding author can add POCs' });
    }
