...
import { createCourseQuizSummaryDocxBlob } from '@/utils/buildCourseQuizSummaryDocx';
import {
  Dialog,
  DialogContent,
  DialogDescription,
...
export default function CourseTopicQuiz() {
  ...
  const questionSummaries = useMemo(
    () =>
      displayQuestions.map((question, index) => {
        ...
        return {
          id: `${topicSlug ?? 'topic'}-summary-${index}`,
          index,
    ...
    [answers, displayQuestions, topicSlug],
  );
  ...
  const downloadSummaryAsWord = async () => {
    if (!topic) return;
    try {
      const blob = await createCourseQuizSummaryDocxBlob({
      ...
      const safeTopicName = topic.title.replace(/[^\w.-]+/g, '_').slice(0, 80) || 'course_topic';
      link.href = objectUrl;
      link.download = `course-quiz-summary-${safeTopicName}-${new Date().toISOString().slice(0, 19).replace(/[:T]/g, '-')}.docx`;
                ...
                <DialogContent className="max-h-[85vh] w-[95vw] max-w-4xl overflow-hidden p-0">
                  <DialogHeader className="flex-row items-start justify-between border-b border-border/60 px-6 py-4 pr-14 text-left">
                    ...
                    <Button
                      ...
                      size="sm"
                      className="gap-2"
                      onClick={() => void downloadSummaryAsWord()}
                    >
