...
type BuildProductQuizSummaryDocxInput = {
  productName: string;
  scoreCorrect: number;
  scoreTotal: number;
  correctItems: ProductQuizSummaryItem[];
  incorrectItems: ProductQuizSummaryItem[];
};
...
function buildQuestionRows(items: ProductQuizSummaryItem[]): TableRow[] {
  if (!items.length) {
    return [
      new TableRow({
        children: [
...
export async function createProductQuizSummaryDocxBlob(
  input: BuildProductQuizSummaryDocxInput,
): Promise<Blob> {
  ...
  const summaryRows: TableRow[] = [
    ...
    dataRow('Product', input.productName),
  ...
  const children: (Paragraph | Table)[] = [
    ...
    new Paragraph({
      alignment: AlignmentType.CENTER,
      spacing: { after: 80 },
      children: [new TextRun({ text: 'PRODUCT QUIZ SUMMARY REPORT', bold: true, size: 32 })],
    }),
    new Paragraph({
      alignment: AlignmentType.CENTER,
      spacing: { after: 240 },
      children: [new TextRun({ text: input.productName, bold: true, size: 28 })],
    }),
    ...
    new Paragraph({
      ...
      children: [
        new TextRun({
          text: 'This report lists detailed product quiz responses, including correct and incorrect answers.',
          size: 18,
          color: '6b7280',
        }),
      ],
